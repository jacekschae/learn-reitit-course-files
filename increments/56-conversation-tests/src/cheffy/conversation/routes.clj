                            ...
                            (let [message (-> request :parameters :body)
                                  ...
                                  conversation-id (conversation-db/dispatch
                                                    [:start-conversation db (assoc message
                                                                              ...
                                                                              :from from)])]
                              (rr/created (str responses/base-url "/v1/conversations" conversation-id
      ...
      {:get  {:handler    (fn [request]
       ...
       :put  {:handler    (fn [request]
                            (let [uid (-> request :claims :sub)
                                  ...
                                  update! (conversation-db/dispatch :clear-notifications db
                                            {:uid uid :conversation-id conversation-id})]
                              (when update!
                                (rr/status 204))))
              :parameters {:path {:conversation-id string?}}
